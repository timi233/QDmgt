#!/usr/bin/env bash
# ============================================
# QDmgt Docker容器启动脚本 (绕过registry mirror)
# ============================================
set -euo pipefail
...
echo -e "${BLUE}🐳 QDmgt Docker容器环境启动 (绕过registry mirror)${NC}"
echo "========================================"
...
# Pull base images directly to avoid registry mirror issues
echo ""
echo "========================================"
echo -e "${YELLOW}📥 预拉取基础镜像...${NC}"
echo "========================================"

# Set environment to bypass mirror registry
export DOCKER_BUILDKIT=1

# Try pulling the base images directly from Docker Hub
echo "拉取基础镜像..."
docker pull python:3.11-slim || echo "⚠️  无法拉取 python:3.11-slim，继续执行..."
docker pull node:18-alpine || echo "⚠️  无法拉取 node:18-alpine，继续执行..."
docker pull nginx:1.25-alpine || echo "⚠️  无法拉取 nginx:1.25-alpine，继续执行..."
docker pull postgres:15-alpine || echo "⚠️  无法拉取 postgres:15-alpine，继续执行..."
...
# 构建镜像 - 使用 DOCKER_BUILDKIT 环境变量
if DOCKER_BUILDKIT=1 $DOCKER_COMPOSE_CMD --env-file .env.docker build; then
