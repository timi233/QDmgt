...
const TargetCreate: React.FC = () => {
  ...
  const handleSubmit = async (values: any) => {
    setLoading(true)
    try {
      await axios.post(`${API_BASE_URL}/targets`, values, {
      ...
      <Card title="新建目标">
        ...
        >
          ...
          <Form.Item
            label="新签目标 (万元)"
            name="newSignTarget"
            rules={[{ required: true, message: '请输入新签目标' }]}
          >
            <InputNumber
              min={0}
              style={{ width: '100%' }}
              placeholder="请输入新签目标金额"
            />
          </Form.Item>
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: '请输入核心商机目标' }]}
          >
            <InputNumber
              min={0}
              style={{ width: '100%' }}
              placeholder="请输入核心商机目标金额"
            />
          </Form.Item>
          ...
          <Form.Item
            label="核心业绩目标 (万元)"
            name="coreRevenue"
            rules={[{ required: true, message: '请输入核心业绩目标' }]}
          >
            <InputNumber
              min={0}
              style={{ width: '100%' }}
              placeholder="请输入核心业绩目标金额"
            />
          </Form.Item>
          ...
          <Form.Item
            label="高价值商机目标 (万元)"
            name="highValueOpp"
            rules={[{ required: true, message: '请输入高价值商机目标' }]}
          >
            <InputNumber
              min={0}
              style={{ width: '100%' }}
              placeholder="请输入高价值商机目标金额"
            />
          </Form.Item>
          ...
          <Form.Item
            ...
            rules={[{ required: true, message: '请输入高价值业绩目标' }]}
          >
            <InputNumber
              min={0}
              style={{ width: '100%' }}
              placeholder="请输入高价值业绩目标金额"
            />
          </Form.Item>
