...
- **TaskDetail.tsx**: 添加任务指派功能
  - 新增"指派"按钮
  - 点击打开指派模态框，可选择用户并填写指派原因
  - 调用 `PUT /api/v1/tasks/:id/assign` API
  - 指派成功后只更新assignedUser，保留其他任务数据
...
#### 前端
- **EditTask.tsx**: 新增任务编辑页面
  - 可编辑字段: 标题、描述、优先级、截止时间、关联经销商
  - 支持清除经销商关联（设为null）
  - 并行获取任务数据和经销商列表

#### 后端
- **taskSchema.ts**: `updateTaskBodySchema` 添加 `distributorId` 字段支持
- **taskService.ts**:
  - `UpdateTaskInput` 接口添加 `distributorId?: string | null`
  - `updateTask` 函数添加经销商验证逻辑
- **taskController.ts**: 移除冗余的 `updateTaskSchema.parse()`，使用已验证的 `req.body`

### 路由
- 添加 `/tasks/:id/edit` 路由到 App.tsx
