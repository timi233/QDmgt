...
echo -e "${BLUE}🐳 QDmgt Docker容器环境启动${NC}"
echo "========================================"
...
# 设置构建环境
export HTTP_PROXY=http://192.168.101.20:7890
export HTTPS_PROXY=http://192.168.101.20:7890
export http_proxy=http://192.168.101.20:7890
export https_proxy=http://192.168.101.20:7890
...
# 构建镜像 - 现在使用镜像站加速
if HTTP_PROXY=$HTTP_PROXY HTTPS_PROXY=$HTTPS_PROXY http_proxy=$http_proxy https_proxy=$https_proxy NO_PROXY=$NO_PROXY no_proxy=$no_proxy $DOCKER_COMPOSE_CMD --env-file .env.docker build --build-arg HTTP_PROXY=$HTTP_PROXY --build-arg HTTPS_PROXY=$HTTPS_PROXY --build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy --build-arg NO_PROXY=$NO_PROXY --build-arg no_proxy=$no_proxy; then
    echo -e "${GREEN}✓${NC} 镜像构建成功"
else
    echo -e "${RED}❌ 镜像构建失败${NC}"
    echo "尝试使用--no-cache重新构建..."
    if HTTP_PROXY=$HTTP_PROXY HTTPS_PROXY=$HTTPS_PROXY http_proxy=$http_proxy https_proxy=$https_proxy NO_PROXY=$NO_PROXY no_proxy=$no_proxy $DOCKER_COMPOSE_CMD --env-file .env.docker build --no-cache --build-arg HTTP_PROXY=$HTTP_PROXY --build-arg HTTPS_PROXY=$HTTPS_PROXY --build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy --build-arg NO_PROXY=$NO_PROXY --build-arg no_proxy=$no_proxy; then
        echo -e "${GREEN}✓${NC} 镜像构建成功（使用--no-cache）"
    else
        echo -e "${RED}❌ 镜像构建仍然失败${NC}"
        exit 1
    fi
fi
