...
def create_channel(
    ...
    try:
        ...
        # 根据错误内容提供更具体的建议
        if "邮箱" in error_message or "email" in error_message.lower():
            if "域名" in error_message or "点" in error_message:
                suggestion = "请输入完整的邮箱地址，域名部分需要包含点号，例如：user@example.com"
            else:
                suggestion = "请输入正确的邮箱格式，例如：user@example.com"
        elif "手机" in error_message or "phone" in error_message.lower():
