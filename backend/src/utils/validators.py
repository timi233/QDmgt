...
def validate_email(email: str) -> bool:
    ...
    if not re.match(email_pattern, email):
        ...
        if '@' not in email:
            ...
            raise ValidationError("邮箱域名不完整，请输入完整的域名（如：example.com）")
        else:
            raise ValidationError("邮箱格式不正确，请输入正确的邮箱地址格式")
...
def validate_phone(phone: str) -> bool:
    ...
    if not re.match(phone_pattern, cleaned):
        if not phone.startswith('+'):
            raise ValidationError("手机号码格式不正确，必须以+开头（如：+8613800138000）")
        else:
            raise ValidationError("手机号码格式不正确，请输入正确的国际手机号码格式")
...
def validate_quarter(quarter: int) -> bool:
    ...
    if quarter < 1 or quarter > 4:
        raise ValidationError("季度值无效，请选择1-4之间的数字（1=第一季度，2=第二季度，3=第三季度，4=第四季度）")
...
def validate_month(month: int) -> bool:
    ...
    if month < 1 or month > 12:
        raise ValidationError("月份值无效，请选择1-12之间的数字（1=1月，2=2月，...，12=12月）")
