...
class AuthService:
    ...
    def create_user(self, db: Session, username: str, email: str, password: str, role: str = "user") -> User:
        ...
        if not is_valid:
            ...
            chinese_errors = []
            for error in errors:
                if "at least" in error and "characters" in error:
                    chinese_errors.append("密码长度至少需要8个字符")
                elif "uppercase" in error:
                    chinese_errors.append("密码需要包含大写字母")
                elif "lowercase" in error:
                    chinese_errors.append("密码需要包含小写字母")
                elif "digit" in error:
                    chinese_errors.append("密码需要包含数字")
                elif "special" in error:
                    chinese_errors.append("密码需要包含特殊字符")
                else:
                    chinese_errors.append("密码强度不够")

            error_message = "；".join(chinese_errors) if chinese_errors else "密码强度不够"
            suggestion = "请输入至少8个字符的密码，建议包含大小写字母、数字和特殊字符以提高安全性"
