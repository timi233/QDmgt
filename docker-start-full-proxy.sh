#!/usr/bin/env bash
# ============================================
# QDmgt Docker容器启动脚本 (完全绕过registry mirror)
# ============================================
set -euo pipefail
...
echo -e "${BLUE}🐳 QDmgt Docker容器环境启动 (完全代理模式)${NC}"
echo "========================================"
...
# Enable BuildKit for better proxy handling
export DOCKER_BUILDKIT=1
export BUILDKIT_PROGRESS=plain

echo "构建后端镜像..."
# Build backend image manually
if docker build -f backend/Dockerfile --build-arg HTTP_PROXY=$HTTP_PROXY --build-arg HTTPS_PROXY=$HTTPS_PROXY --build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy --build-arg NO_PROXY=$NO_PROXY --build-arg no_proxy=$no_proxy -t qdmgt_backend .; then
...
# For the frontend, we need to handle it differently since it's in a subdirectory
echo "构建前端镜像..."
if docker build -f frontend/Dockerfile --build-arg HTTP_PROXY=$HTTP_PROXY --build-arg HTTPS_PROXY=$HTTPS_PROXY --build-arg http_proxy=$http_proxy --build-arg https_proxy=$https_proxy --build-arg NO_PROXY=$NO_PROXY --build-arg no_proxy=$no_proxy -t qdmgt_frontend ./frontend; then
...
# 启动服务 with proxy variables
if DOCKER_BUILDKIT=1 $DOCKER_COMPOSE_CMD --env-file .env.docker up -d --build; then
